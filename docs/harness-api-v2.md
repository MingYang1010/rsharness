...
V2 startup applies transactional, idempotent migrations to `/sata/yangm/eo-harness/state/episodes.sqlite3`. Migration failure does not advance the V2 schema version or leave partial V2 tables. The artifact mount is `/sata/yangm/eo-harness/artifacts:/app/artifacts`; content is written atomically by SHA-256 and audited before reads. Store schema `3` rebuilds `v2_evidence` with primary key `(episode_id, evidence_id)`; evidence IDs are episode-local, while duplicate IDs within one episode still return HTTP `409 evidence_conflict`. Store schema `4` rebuilds `v2_artifacts` with primary key `(episode_id, artifact_id)`; deterministic derivation IDs may recur across episodes, while mismatched metadata in one episode still conflicts. Rolling back the application before schemas `3`/`4` requires a compatible store migration or restoring the pre-migration SQLite backup; do not delete V2 tables or artifact files.
