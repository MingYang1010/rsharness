...
| G07 / P1 记忆收益与污染 | 2026-09-26 `runtime/evidence-memory-matrix-v4-20260926/matrix-run/matrix-manifest.json` 完成真实 Qwen 8-job 矩阵：correct-only 与邻区均检索唯一目标记录并正确提交；expired-only 返回 0 后正确 abstain；conflict 返回 2 条后模型无 action（runner 保留为 `model_returned_text_without_action`）。报告含逐 episode 检索 ID、action、token 与时延 | 传感器/日期不适用的检索仍未覆盖。expired-only 当前 evaluator 期望正确 memory ID，行为正确但 reward 为 0，需单独定义 no-valid-memory abstention。conflict 需要扩展 max turns 或显式冲突裁决动作，并把模型语义失败与 harness 失败分开统计 |
