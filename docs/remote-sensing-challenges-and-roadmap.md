...
| D / P1 决策任务包 | 已在 `20bd023` 完成 evaluator 第一段：WHU 支持 `expected_outcome`、`answer.abstention_correctness`，并记录 false-confidence/unnecessary-abstention；后续继续冻结变化判断，再加时序选择；问题、独立 reference、输入可用性、预算、split、评价器一并固定 | 开发集排障后锁定 held-out cases；基于观测的后续动作可改变结论；标签不进入 Agent/runtime context | 第一段已提交：`feat(evaluation): score WHU answerability outcomes`；任务包后续：`feat(benchmarks): add observation-dependent tasks` |
