#!/usr/bin/env python3
"""Admit reviewed SCL/reference pairs from an existing Sentinel receipt."""
import argparse
import hashlib
import json
import re
...
from app.v2.raster_grid import NativeSCL, checked_grids
from app.v2.raster_math import MAX_INPUT, NativeBand
...
def prepare(source: Path, out: Path):
    ...
    if receipt["status"] != "admitted" or len(receipt["windows"]) != 12:
        raise ValueError("three reviewed dates required")
    records = [r for r in receipt["windows"] if r["asset_key"] in {"red", "scl"}]
    if len(records) != 6:
        raise ValueError("six reviewed grid inputs required")
    with StorageQuota(ROOT / "runtime").hold(out, 128 * 1024 * 1024, "grid-task-admission"):
        for name in ("inputs", "native-inputs", "state", "reports", "tasks/scl-grid"):
            (out / name).mkdir(parents=True)
        template = ROOT / "tasks/worldcover-grounded-vqa"
        assets, profiles, provider, pairs = [], {}, {}, {}
        for record in records:
            ...
            with rasterio.open(path, driver="GTiff") as image:
                if (image.count != 1 or image.dtypes != (record["dtype"],)
                        or image.crs.to_string() != record["crs"]
                ...
                common = dict(asset_id=asset_id, sha256=record["sha256"], item_id=record["item_id"],
                    acquired=record["acquired"], crs=record["crs"], transform=record["transform"],
                    width=image.width, height=image.height, dtype=record["dtype"],
                    scale=image.scales[0], offset=image.offsets[0], nodata=image.nodata)
                profile = NativeSCL(band="scl", **common) if record["asset_key"] == "scl" else NativeBand(band="red", **common)
            with (out / "native-inputs" / filename).open("xb") as stream:
                stream.write(content)
            profiles[asset_id] = profile.model_dump(mode="json")
            provider[asset_id] = {"filename": filename, "native": profiles[asset_id]}
            pairs.setdefault(profile.item_id, {})[profile.band] = profile
            west, south, east, north = record["bbox_wgs84"]
            roles = ["input_image", "scene_classification"] if profile.band == "scl" else ["input_image", "reflectance"]
            assets.append({"asset_id": asset_id, "uri": "local://approved-grid/" + filename,
                "media_type": "image/tiff", "roles": roles, "sha256": record["sha256"],
                "size_bytes": len(content), "spatial": {"crs": "EPSG:4326",
                    "bbox": dict(west=west, south=south, east=east, north=north),
                    "gsd_meters": profile.transform[0], "shape": [profile.height, profile.width, 1]},
                ...
                "source": "Reviewed Sentinel-2 SCL or reference reflectance window",
                "source_snapshot_hash": record["source_snapshot_hash"]})
        for pair in pairs.values():
            checked_grids(pair["scl"], pair["red"])
        if len(pairs) != 3:
            raise ValueError("three unique pairs required")
        ...
        task.update(task_id="scl-grid-" + identity, inputs=[a["asset_id"] for a in assets],
            prompt="Find each same-date SCL/red pair and align 20 m SCL to the red 10 m grid using explicit nearest-neighbor. Cite all three categorical rasters and report valid coverage plus SCL class counts. Class 0 and source-invalid pixels are invalid; no cloud mask is applied and SCL is not change ground truth.",
            metadata={"observation_profile": "headless-tools-v1",
                      "artifact_identity": "derivation-sha256-v1",
                      "acceptance": "scripted-scl-grid-numeric-check", "grid_inputs": profiles})
        task["budget"].update(max_steps=40, max_tool_calls=25, max_wall_time_ms=600000,
                              max_input_bytes=256 * 1024 * 1024, max_artifact_bytes=64 * 1024 * 1024)
        ...
        scenario.update(data_cutoff=receipt["config"]["cutoff"], freshness_max_age_seconds=None,
            ...
            allowed_tools=["catalog.search", "catalog.inspect_asset", "raster.resample"])
        directory = out / "tasks/scl-grid"
        ...
        write(out / "job.json", {"seed": 42,
              "task_ref": {"task_id": task["task_id"], "task_version": task["task_version"]}})
        ...
        write(out / "admission.json", {
            ...
            "task_manifest_hash": manifest.task_manifest_hash, "inputs": 6, "date_pairs": 3,
            "method": "nearest", "cloud_mask_applied": False})
