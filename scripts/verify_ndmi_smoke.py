#!/usr/bin/env python3
"""Agent-only acceptance for fixed NDMI followed by pinned-zone statistics."""
...
def main() -> None:
    ...
    checkpoint = Path("/reports/ndmi-checkpoint.json")
    with httpx.Client(
        ...
        if arguments.resume:
            ...
            report = {"status": "passed",
                      ...
                      "raster_artifacts": len(saved["artifacts"]),
                      "denied": denied}
            Path("/reports/ndmi-resume.json").write_text(
        ...
        assert {"raster.resample", "raster.band_math",
                "raster.zonal_stats"}.issubset(session["tool_schemas"])

        def step(action):
            nonlocal state
            body = {"client_action_id": "fixed-ndmi-" + str(len(actions)),
        ...
        aligned_ref = grid_response["observation"]["items"][1]["artifact_ref"]
        aligned = request("GET", "/agent/artifacts/" + aligned_ref)["artifact"]
        artifacts.append(aligned)

        ndmi_response = tool("raster.band_math", {
            "operation": "ndmi", "nir_asset_id": pair["nir"]["asset_id"],
            "swir_artifact_id": aligned_ref})
        ndmi_result = ndmi_response["observation"]["items"][0]["inline"]
        assert (ndmi_result["tool_version"] == "1.2.0"
                and ndmi_result["operation"] == "ndmi"
                and ndmi_result["formula_id"]
                == "sentinel-2-ndmi-nir-swir16-v1"
                and ndmi_result["swir_artifact_id"] == aligned_ref
                and -1 <= ndmi_result["minimum"] <= ndmi_result["mean"]
                <= ndmi_result["maximum"] <= 1)
        ndmi_ref = ndmi_response["observation"]["items"][1]["artifact_ref"]
        ndmi = request("GET", "/agent/artifacts/" + ndmi_ref)["artifact"]
        artifacts.append(ndmi)

        zonal_response = tool("raster.zonal_stats", {
            "raster_artifact_id": ndmi_ref, "zone_id": ZONE_ID})
        zonal = zonal_response["observation"]["items"][0]["inline"]
        assert (zonal["tool_version"] == "1.1.0"
                and zonal["source_artifact_id"] == ndmi_ref
                and zonal["source_sha256"] == ndmi["sha256"]
        ...
        step({"type": "memory.save_evidence", "evidence": {
            "evidence_id": "ev-fixed-ndmi", "claim_id": "fixed-ndmi",
            "source_ref": ndmi_ref,
            "selector": {"bbox": bbox, "time_range": ndmi["temporal"]},
            "frozen_sha256": ndmi["sha256"],
            "description": ("Fixed Sentinel-2 NDMI and pinned polygon "
                            "statistics over the B08 grid")}})
        final = step({"type": "answer.submit",
                      "answer": {"label": "fixed-ndmi", "confidence": 1.,
                                 "claims": [{"claim_id": "fixed-ndmi",
